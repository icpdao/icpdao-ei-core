...
def test_2():
    issues = json.load(open(os.path.join(test_path, 'data/2_issues.json')))
    ...
    ep = EiProcessor('2_issues', ei_issue_list)
    ep.process()
    assert ep.pair_success() == True
    # json.dump(ep.assignees_info, open(os.path.join(test_path, 'data/period_one_issue_assignees_info.json'), "w"), ensure_ascii=False, indent=True)
